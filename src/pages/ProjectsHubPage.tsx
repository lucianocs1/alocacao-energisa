...
import { ChevronDown, ChevronRight, Plus, Calendar, Users, Clock, AlertTriangle, CheckCircle2, CircleDot, ArrowRightCircle, Diamond } from 'lucide-react';
...
import { Project, Demand, DEMAND_STATUS_CONFIG, MONTHS, PHASE_CONFIG } from '@/types/planner';
...
const ProjectsHubPage = () => {
        ...
        <div className="w-32">
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge 
                variant="outline" 
                className={`${team?.color.replace('bg-', 'border-')} ${team?.color.replace('bg-', 'text-').replace('-500', '-700')}`}
              >
  ...
  const allDemands = mockProjects.flatMap(p => p.demands);
  const globalStats = {
